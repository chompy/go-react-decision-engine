...
export default class PdfBuilder {
    ...
    static nodeListToPdfMake(nodeList, params) {
        ...
        for (let i = 0; i < nodeList.length; i++) {
            ...
            switch (node.nodeName.toLowerCase()) {
                ...
                case 'b': 
                {
                    let styles = PdfBuilder.getStyles(node);
                    if (!styles._visible) { return out; }
                    out = out.concat(
                        PdfBuilder.nodeListToPdfMake(
                            node.childNodes,
                            Object.assign(
                ...
                case 'h6':
                {
                    ...
                    let fontSize = PdfBuilder.headerFontSizes[header-1];
                    out.push(
                        PdfBuilder.buildFilledBox(
                            Object.assign(
                ...
                case 'p':
                {
                    ...
                    for (let j = 0; j < node.childNodes.length; j++) {
                        if (PdfBuilder.inlineElements.indexOf(node.childNodes[j].nodeName.toLowerCase()) == -1) {
                            isInline = false;
                            break;
                        }
                    }
                    if (isInline) {
                        out.push(
                            PdfBuilder.buildFilledBox(
                                Object.assign(
                                    {
                                        text: PdfBuilder.nodeListToPdfMake(node.childNodes, params)
                                    },
                                    styles
                                )                 
                            )
                    ...
                    out.push(
                        PdfBuilder.buildFilledBox(
                            Object.assign(
                                {
                                    stack: PdfBuilder.nodeListToPdfMake(node.childNodes, params)
                                },
                                styles
                            )                 
                        )
                ...
                case 'table':
                {
                    ...
                    for (let j = 0; j < trList.length; j++) {
                        ...
                        for (let k = 0; k < trList[j].childNodes.length; k++) {
                            ...
                            let styles = PdfBuilder.getStyles(trList[j].childNodes[k]);
     ...
     */
    static build(obj, name, userData) {
        ...
        let def = {
            pageSize: 'LETTER',
            content: prependContentDef.concat(obj.toPdfMake(userData), appendContentDef)
        };
        if (fonts.length > 0) {
            def['defaultStyle'] = Array.keys(fonts)[0];
        }
        if (!name && typeof(obj.name) != 'undefined' && obj.name) {
            name = encodeURI(obj.name.replace(' ', '-'));
        } else if (!name) {
            name = obj.uid;
        }
        pdfMake.createPdf(
        ...
        Logger.infoTime(`Generated PDF. [UID=${obj.uid}]`, 'ccde_pdf');
    }
