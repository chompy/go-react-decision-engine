...
func getObjectTeam(i interface{}) (primitive.ObjectID, error) {
	switch i := i.(type) {
	case *TreeRoot:
		{
			if i.Parent.IsZero() {
				return primitive.NilObjectID, ErrObjMissingParam
			}
			switch i.Type {
			case TreeForm:
				{
			...
			case TreeDocument:
				{
					...
					return treeForm.(*TreeRoot).Parent, nil
				}
			default:
				{
					if i.Type == "" {
					...
					return primitive.NilObjectID, ErrObjInvalidParam
				}
			}
		}
	case *TreeVersion:
		{
			...
			return treeRoot.(*TreeRoot).Parent, nil
		}
	case *FormSubmission:
		{
	...
	return primitive.NilObjectID, ErrDBInvalidObjectType
}
